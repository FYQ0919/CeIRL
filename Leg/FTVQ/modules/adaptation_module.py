...
class MLPHistoryEncoder(nn.Module):
    def __init__(self, 
        ...
        for l in range(len(adaptation_module_branch_hidden_dims)):
            if l == len(adaptation_module_branch_hidden_dims) - 1:
                adaptation_module_layers.append(
                    nn.LSTM(adaptation_module_branch_hidden_dims[l], output_size, batch_first=True))
            else:
                adaptation_module_layers.append(
                    nn.LSTM(adaptation_module_branch_hidden_dims[l], adaptation_module_branch_hidden_dims[l + 1],
                            batch_first=True))
                adaptation_module_layers.append(activation)
        self.encoder = nn.Sequential(*adaptation_module_layers)
