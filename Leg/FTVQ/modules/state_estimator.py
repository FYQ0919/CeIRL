...
class VAE(nn.Module):
    def __init__(
        ...
        self.encoder = encoder(
            (num_obs + num_latent) * num_history,
            self.num_latent * 2 + 6,
            activation,
            encoder_hidden_dims,
        )
    ...
    def forward(self, obs_history, cv):
        ...
        latent_var = torch.clip(latent_var, -32767.0 , 1.0)
        vel_var = torch.clip(vel_var, -32767.0 , 1.0)
    ...
    def loss_fn(self, latent, obs_history, obs_next, vel, cv, kld_weight=1.0):
        latent_and_history = torch.cat([latent, obs_history], dim=-1)
        [z, v], [latent_mu, latent_var, vel_mu, vel_var] = self.forward(latent_and_history, cv)
