...
from FTVQ.algorithms import PPO
from FTVQ.modules import ActorCritic
from FTVQ.utils import VecEnv
...
class OnPolicyRunner:
    ...
    def learn(
        ...
        for it in range(
            ...
            mean_value_loss, mean_surrogate_loss,mean_entropy_loss,mean_recons_loss, mean_vel_loss, mean_kld_loss, mean_forward_loss, mean_contrastive_loss = self.alg.update()
            stop = time.time()
            learn_time = stop - start
            if len(rewbuffer) >= 2:
                self.cv = statistics.stdev(rewbuffer)
            print(self.cv)
            if self.log_dir is not None:
                self.log(locals())
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar('Loss/Forward', locs['mean_forward_loss'], locs['it'])
        ...
        if len(locs['rewbuffer']) > 0:
            ...
            plot_names = [(self.std_his,"action_std"),(self.rew_his,"reward")]
        # if locs['it'] % 25 == 1:
        #     for data, title in plot_names:
        #         plt.figure(figsize=(10, 5))
        #         plt.plot(self.step, data, label=title)
        #         plt.title(title)
        #         plotname = f"{title}_step.png"
        #         filepath = os.path.join(self.log_dir, plotname)
