...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: BasicCfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None,
        ...
        self.rew_bonus_terrain = None
        self.need_damage_leg = True
        self.damage_min = 0.
        self.damage_max = 0.001
    ...
    def step(self, actions):
        ...
        # self.actions[:,11] = self.last_actions[:,11]
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            if self.need_damage_leg:
                # if end - self.start_time > 50:
                #     print("mask start")
                # self.torque_mask[:, 11] = 0.5
                    # self.torque_mask[:, 6] = 0
                    # self.torque_mask[:, 10] = 0
                # print(self.torque_mask)
                self.torques = self.torques * self.torque_mask
                # print(self.torque_mask[0])
    ...
    def check_termination(self):
        ...
        """
        self.reset_buf = torch.any(torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 400.,
                                   dim=1)
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.terrain.curriculum:
            self.rew_bonus_terrain = self.terrain_curriculum.update_curriculum(env_ids)
            self.env_origins[env_ids] = self.terrain_curriculum.sample_terrain(env_ids)

        if not self.eval:
            self._resample_commands_v1(env_ids)
        ...
        if self.need_damage_leg:
            ...
            self._reset_damage_legs(env_ids)
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            # if self.rew_bonus_terrain != None and self.reward_scales[name] > 0:
            #     rew = rew * (self.rew_bonus_terrain ** 0.5 * 0.3 + 1)
            self.rew_buf += rew
            # print(self.reward_names[i],rew.mean())
            if torch.sum(rew) >= 0:
    ...
    def compute_observations(self):
        ...
        if self.cfg.env.priv_observe_torque_mask:
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                 self.damage_leg_info),
                                                dim=-1)
    ...
    def _compute_torques(self, actions):
        ...
        if control_type == "actuator_net":
            ...
            torques[:,1] = 1000 * (
                    1.0 - self.dof_pos[:,1] + self.motor_offsets[:,1])
        elif control_type == 'T':
            torques = actions
        else:
...
#
    ...
    def _reset_damage_legs(self, env_ids):
        for i in env_ids:
            # if np.random.uniform(0, 1, size=1) > 0.5:
                self.torque_mask[i] = torch.ones(self.num_dof, dtype=torch.float, device=self.device,
                                                 requires_grad=False)
                # indices = np.array(random.sample(range(self.num_dofs), 1))
                # self.torque_mask[i, indices] = 0
                # k = random.randint(0, self.num_damage_joints)
                # if k > 0:
                indices = np.array(random.sample(range(self.num_dofs),1))
                damage_strength = torch.tensor(
                    np.random.uniform(low=self.damage_min, high=self.damage_max, size=1), dtype=torch.float,
                    device=self.device, requires_grad=False)
                self.torque_mask[i, indices] = damage_strength
                # self.torque_mask[i, 1] = damage_strength
                # self.torque_mask[i, :3] = 0
                self.damage_leg_info[i] = torch.ones(self.num_dof, dtype=torch.float, device=self.device,
                                                     requires_grad=False)
                # self.damage_leg_info[i, :3] = 0
                if damage_strength < 0.1:
                    self.damage_leg_info[i, indices] = 0
    ...
    def _init_custom_buffers__(self):
        ...
        self.torque_mask = torch.ones(self.num_envs, self.num_dof, dtype=torch.float, device=self.device,
                                      requires_grad=False)
        self.damage_leg_info = torch.ones(self.num_envs, self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
