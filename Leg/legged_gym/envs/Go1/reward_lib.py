...
class RewardLib:
    ...
    def _reward_collision_force(self):
        # Penalize collisions on selected bodies
        return torch.sum(torch.norm(self.env.contact_forces[:, self.env.penalised_contact_indices, :], dim=-1))
    ...
    def _reward_hip_pos(self):
        return torch.sum(torch.square(self.env.dof_pos[:, self.env.hip_indices] - self.env.default_dof_pos[:, self.env.hip_indices]),
                         dim=1)
    ...
    def _reward_feet_clearance(self):
        phases = 1 - torch.abs(1.0 - torch.clip((self.env.feet_indices * 2.0) - 1.0, 0.0, 1.0) * 2.0)
    ...
    def _reward_foot_clearance(self):
        self.env.measured_foot_clearance
